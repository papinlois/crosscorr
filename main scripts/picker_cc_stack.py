...
filtered_sta = [station for station in all_sta if station in {'SSIB', 'SILB', 'TSJB', 'TWKB', 'KLNB'}]
...
def detc_sta(sta):
    ...
    # nb_stack=[100,50,20,10,5,2,1]
    nb_stack=[450,350,200]
    for nb in nb_stack:
        ...
        for templ in nb_templ:
            ...
            if len(events_templ)>=nb:
                ...
                #--- Parameters ---
                run_num = 'S003'  # Run number identifier
                ...
                #--- Determine Station Network and Parameters ---
                ...
                #--- Create Output Files ---
                file_csv = './Detections_S_PO_stack/cut_daily_%s.%s.csv' % (net, sta)
                # file_hdf5 = './Detections_S_PO_stack/cut_daily_%s.%s.hdf5' % (net, sta)
                # Create CSV file
                if not(os.path.exists(file_csv)):
                    OUT1 = open(file_csv,'w')
                    OUT1.write('nb_stack,mean_cc,lfe_family,S_times,y,idx_max,id\n')
                    OUT1.close()
                # else:
                ...
                stlon, stlat, stdep = stainfo[sta]  # Retrieve station location
                ...
                for i_lfe in idx_lfe:
                    ...
                    S_time=wid_T[idx_maxy]
                    tr_id = '.'.join([net, sta, chn]) 
                    OUT1.write('%d,%.2f,%s,%.2f,%.2f,%d,%s\n' % (nb, mean_cc, templ, S_time, tmp_y[i_lfe][idx_maxy], idx_maxy, tr_id))
